...
public class RecuperoClaveOB extends ApiObjeto {

	private static final String CLU_VERSION_ENC = "SHA-512";
	...
	/* ========== SERVICIOS ========== */
	// API-Seguridad_RecuperarClaveOB
	public static RecuperoClaveOB post(Contexto contexto, String numeroDocumento, String usuario, String clave) {
		String claveEncriptada = Encriptador.sha512(clave);

		ApiRequest request = new ApiRequest("RecuperoClaveOB", "seguridad", "POST", "/v1/recuperarclaveob", contexto);
		...
		request.body("clu_version_enc", CLU_VERSION_ENC);
		...
		ApiException.throwIf("CLAVE_INVALIDA", response.contains("Password nueva utilizada anteriormente"), request, response);
		ApiException.throwIf(!response.http(200), request, response);
		return response.crear(RecuperoClaveOB.class, response);
	}

	/* ========== TEST ========== */
	public static void main(String[] args) throws InterruptedException {
		Contexto contexto = contexto("OB", "desarrollo");
		RecuperoClaveOB datos = post(contexto, "95904456", "Julieta3", "Banco123");
