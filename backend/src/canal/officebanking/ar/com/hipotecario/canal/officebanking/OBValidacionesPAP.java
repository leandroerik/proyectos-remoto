...
public class OBValidacionesPAP extends ModuloOB {
    ...
    public static Objeto validarHeader(ArchivoPapDTO.Header header, SubConveniosOB.SubConvenio subConvenio) {
        ErrorArchivoOB errores = new ErrorArchivoOB();
        ...
        if (!header.idArchivo.startsWith("PO")) {
            return errores.setErroresArchivo("ID archivo invalido", "ID de archivo debe ser'PO'", 0, null);
        }
        ...
        if (!header.idContenido.equals("ORDENDEPAGO")) {
            return errores.setErroresArchivo("ID contenido invalido", "ID contenido debe ser 'ORDENDEPAGO'", 0, null);
        }
        ...
        if (!header.loteCompleto.equals("N")) {
            return errores.setErroresArchivo("Procesa por lote completo invalido", "Procesa por lote completo debe ser 'N'", 0, null);
        }

        if (header.TipoOperatoria.isBlank()
                || (!header.TipoOperatoria.equals("P") &&
                !header.TipoOperatoria.equals("G") &&
                !header.TipoOperatoria.equals("T") &&
                !header.TipoOperatoria.equals("J") &&
                !header.TipoOperatoria.equals("O") &&
                !header.TipoOperatoria.equals("S"))) {
            return errores.setErroresArchivo("Tipo de operatoria invalido", "El tipo de operatoria debe ser 'P','G','T','J','O' o 'S'", 0, null);
        }
//        if (!header.fechaEjecucion.isBlank()) {
//            return errores.setErroresArchivo("Fecha de ejecucion invalida", "Fecha de ejecucion debe estar vacia", 0, null);
//        }
//        if (!header.cbu.isBlank()) {
//
//            return errores.setErroresArchivo("CBU Invalido", "CBU debe estar vacio", 0, null);
//        }
//        if (!header.moneda.isBlank()) {
//            return errores.setErroresArchivo("Moneda invalida", "Moneda debe estar vacio", 0, null);
//        }
...
//        }
//
        if (!item.cbuCredito.isBlank() && !item.cbuCredito.matches("^[0-9]{22}$")) {
            return errores.setErroresArchivo("CBU cuenta destino", "El campo CBU de la cuenta destino debe ser numérico y de 22 caracteres.", numLinea, null);
        }
//
