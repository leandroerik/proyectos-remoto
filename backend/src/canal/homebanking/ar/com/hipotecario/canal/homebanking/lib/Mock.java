...
public class Mock {
	...
	public static ApiResponse resumenCuentaTCMock(ApiResponse responseResumen, String cobis) {
	...
	public static ApiResponse replaceResumenCuentaTC(ApiResponse response, Integer proximoLiqVenc, Integer proximoLiqcierre, Integer ultimaLiqVenc, Integer ultimaLiqcierre) {
		...
		try {
			...
			return new ApiResponse(null, response.codigo, json);
		} catch (Exception e) {
			return new ApiResponse(null, 204, json);
		}
	}
	...
	public static String aumentarFechaHora(String fechaString, int cantidadDias) {
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss");
		LocalDateTime fecha = LocalDateTime.parse(fechaString, formatter);
		return fecha.plusDays(cantidadDias).plusHours(cantidadDias).format(formatter);
	}
}
