...
public class CuentaTercero {
    ...
    /* ========== ATRIBUTOS ========== */
    public String id;
    public ApiResponseMB cuentaBH = new ApiResponseMB();
    public ApiResponseMB cuentaLink = new ApiResponseMB();
    public ApiResponseMB cuentaCoelsa = new ApiResponseMB();
    public ApiResponseMB cuentaCbuBH = new ApiResponseMB();
    public ContextoMB contexto;
    public Boolean cuentaEncontrada = false;

    /* ========== CONSTRUCTOR ========== */
    public CuentaTercero(ContextoMB contexto, String id) {
        this.id = id;
		...
		if (CuentasService.esCbu(id)) {
			...
			if (cuentaLink.hayError()) {
				ApiResponseMB cuentaLinkAux = CuentasService.cuentaLink(contexto, id, "80");
		...
		if (CuentasService.esAlias(id)) {
			...
			if (!cuentaCoelsa.hayError()) {
				...
				if (!CuentasService.esCvu(cbu)) {
					...
					// 80.
					if (cuentaLink.hayError()) {
						ApiResponseMB cuentaLinkAux = CuentasService.cuentaLink(contexto, cbu, "80");
						if (!cuentaLinkAux.hayError()) {
							cuentaLink = cuentaLinkAux;
						}
					}
    ...
    public CuentaTercero(ContextoMB contexto, String id, boolean v2) {
        this.id = id;
        this.contexto = contexto;
        if (CuentasService.esCbu(id)) {
            cuentaEncontrada = true;
	...
	public String idMoneda() {
		...
		if (idMoneda == null && CuentasService.esAlias(id) && cbu != null) {
	...
	public Set<String> idMonedas() {
		...
		if (idMonedas.contains("2")) {
			...
			ApiResponseMB response = CuentasService.cuentaLink(contexto, cadena, "80");
    ...
    public String codigoBanco() {
        String tipo = null;
        tipo = tipo == null && CuentasService.esCbu(id) ? (StringUtils.isNotBlank(cuentaLink.string("codigoBancoDestino")) ? cuentaLink.string("codigoBancoDestino").substring(1) : tipo) : tipo;
        tipo = tipo == null && CuentasService.esCvu(id) ? null : tipo;
        tipo = tipo == null && CuentasService.esCuentaBH(id) ? "044" : tipo;
        tipo = tipo == null && CuentasService.esAlias(id) ? (StringUtils.isNotBlank(cuentaLink.string("codigoBancoDestino")) ? cuentaLink.string("codigoBancoDestino").substring(1) : tipo) : tipo;
        if (tipo == null || tipo.isEmpty())
    ...
    public boolean esCuentaBroker() {
        return SqlHomebanking.getBroker(this.cuit(), EstadoBroker.ACTIVO) != null;
    }


    public Boolean esAlias() {
		return CuentasService.esAlias(id);
	}
