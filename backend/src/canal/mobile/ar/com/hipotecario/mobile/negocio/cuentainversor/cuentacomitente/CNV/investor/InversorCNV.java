...
public class InversorCNV {
    ...
    public static List<InversorCNV> getInversorCNVCoTitulares(ContextoMB contexto, Cuenta cuentaPesos) {
        List<InversorCNV> inversorCNVList = new ArrayList<>();
//        Cliente clienteCotitular;
        ApiResponseMB responseProducto = ProductosService.integrantesProducto(contexto, cuentaPesos.numero());
...
//        inversorCNVList = getInversorCNVCoTitulares();
    ...
    private static InversorCNV getCotitularesByCliente(ContextoMB contexto ,Cliente cliente){
        ApiMB.eliminarCache(contexto, "Email", contexto.idCobis());
        InversorCNV inversorCNV = new InversorCNV();
        ApiResponseMB responseMail = RestPersona.emails(contexto,cliente.cuit);
...
//                .orElse(null);
        ...
        inversorCNV
                ...
                .setIdentificadorInteresado(
                        Objeto.listOf(
                                new IdentificadorInteresado(
                                        ...
                                        "ARGENTINA",
                                        "00" //persona.idPaisResidencia() return 80
                                )
                        )
                )
                .setNacionalidad(RestCatalogo.nombrePais(contexto, Integer.valueOf(cliente.idNacionalidad)))//todo: eliminar cache primero
                ...
                .setPaisRegistro("ARGENTINA")
                .setPaisResidencia(RestCatalogo.nombrePais(contexto, 80)) //persona.idPaisResidencia()
    ...
    public static InversorCNV getInversorCNVTitular(ContextoMB contexto , Persona persona, Domicilio domicilio) {
        ...
        Integer codigoPostal = contexto.parametros.integer("codigoPostal", null);
        Integer nacionalidad = contexto.parametros.integer("nacionalidad", contexto.persona().idPaisNacimiento());
        ...
        try{
            inversorCNV
                    ...
                    .setCodigoAdministrador("")     //dato fijo 139 homo 1270 prod
                    .setCodigoInversor(new BigInteger(persona.cuit()))
                    .setDetallesContacto(
                            Objeto.listOf(new DetalleContato(
                    ...
                    .setDetallesDireccion(
                            Objeto.listOf(new DetalleDireccion(
                                    ...
                                    , RestCatalogo.nombrePais(contexto, persona.idPaisResidencia())
                                    , "MAIN")
                            )
                    )
                    .setEsInversorParticipante(true)
                    ...
                    .setNacionalidad(RestCatalogo.nombrePais(contexto, nacionalidad))
                    .setNombre(persona.nombres())
                    .setNumIdentificador(new BigInteger(persona.cuit()))
                    .setPaisDomicilioFiscal("ARGENTINA") //todo: NO LO ENVIA EL FRONT
                    .setPaisRegistro("ARGENTINA")
                    .setPaisResidencia(RestCatalogo.nombrePais(contexto, persona.idPaisResidencia()))
                    .setTipoActivacion("ACTIVE") // Fijo
                    .setTipoInversor("Individual");
        }catch (Exception e){
            return null;
