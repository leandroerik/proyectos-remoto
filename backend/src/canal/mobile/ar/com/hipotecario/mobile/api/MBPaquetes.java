...
import ar.com.hipotecario.mobile.negocio.Paquete;
import ar.com.hipotecario.mobile.negocio.TarjetaCredito;
import ar.com.hipotecario.mobile.negocio.TarjetaDebito;
import ar.com.hipotecario.mobile.servicio.PaquetesService;
import ar.com.hipotecario.mobile.servicio.ProductosService;

public class MBPaquetes {

	public static RespuestaMB consolidadaPaquetes(ContextoMB contexto) {
		...
		if (existePaquete) {
			...
			if ("".equals(numeroPaquete)) {
				...
				ApiResponseMB responseConsolidadaPaquetes = PaquetesService.consolidadaPaquetes(contexto, numeroPaquete);
				...
				for (Objeto item : responseConsolidadaPaquetes.objetos()) {
					if ("AHO".equals(item.string("tipo")) || "CTE".equals(item.string("tipo"))) {
						if (item.bool("cuentaCobro")) {
							...
							if (cuenta == null) {
								return RespuestaMB.estado("ERROR_SIN_CUENTA_COBRO");
							}
				...
				ApiResponseMB responseBonificacionesVigentes = PaquetesService.bonificacionesVigentesPorPaquetes(contexto, codigoPaquete, numeroPaquete);
				...
				ApiResponseMB responseBonificacionesObjetivo = PaquetesService.bonificacionesHistoricoUltimoCobro(contexto, numeroPaquete);
				...
				ApiResponseMB responseHistorico = PaquetesService.bonificacionesHistorico(contexto, numeroPaquete, "", "");
				...
				datosPrincipales.set("mostrarTextoBonificacion", !"0".equals(ultimoPorcentajeBonificacionHistorico.replace(" ", "").replace("%", "")));
	...
	public static RespuestaMB historicoBonificaciones(ContextoMB contexto) {
		...
		ApiResponseMB responseHistorico = PaquetesService.bonificacionesHistorico(contexto, numeroPaquete, fechaDesde, fechaHasta);
	...
	public static RespuestaMB historicoBonificacionesDetalle(ContextoMB contexto) {
		...
		Objeto bonificaciones = new Objeto();
		ApiResponseMB responseBonificaciones = PaquetesService.bonificacionesHistoricoDetallado(contexto, numeroPaquete, fecha, servicio, rubro);
	...
	private static String traerNumeroPaquete(ContextoMB contexto) {
		Boolean existePaquete = false;
		String numeroPaquete = "";
