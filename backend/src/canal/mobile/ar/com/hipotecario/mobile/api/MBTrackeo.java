...
import ar.com.hipotecario.mobile.servicio.RestDelivery;

public class MBTrackeo {
	...
	public static RespuestaMB agregarTrackeoTarjetaDebito(ContextoMB contexto) {
		RespuestaMB respuesta = new RespuestaMB();
		if (MBAplicacion.funcionalidadPrendida(contexto.idCobis(), flagTrackeo)) {
			String dni = contexto.persona().numeroDocumento();
			ApiResponseMB response = RestDelivery.deliveryClientes(contexto, dni);
	...
	private static List<String> getPendingIds() {
		return Arrays.asList(ConfigMB.string("delivery_pending_id").split("_"));
	}

	private static List<String> getDepartedIds() {
		return Arrays.asList(ConfigMB.string("delivery_departed_id").split("_"));
	}

	private static List<String> getDelayedIds() {
		return Arrays.asList(ConfigMB.string("delivery_delayed_id").split("_"));
	}

	private static List<String> getLocationIds() {
		return Arrays.asList(ConfigMB.string("delivery_location_id").split("_"));
	}

	private static List<String> getDeliveredIds() {
		return Arrays.asList(ConfigMB.string("delivery_delivered_id").split("_"));
	}

	private static List<String> getRefusedIds() {
		return Arrays.asList(ConfigMB.string("delivery_refused_id").split("_"));
	}
	...
	private static String obtenerTexto(String clave, String idCobis) {
		String config = ConfigMB.string(clave);

		if (MBAplicacion.funcionalidadPrendida(idCobis, flagTrackeoCodificacion) && config != null)
