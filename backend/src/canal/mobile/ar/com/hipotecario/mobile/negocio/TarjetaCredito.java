...
public class TarjetaCredito {
	...
	public TarjetaCredito(ContextoMB contexto, Objeto tarjetaCredito) {
	...
	public String idEncriptado() {
		return "true".equals(ConfigMB.string("prendido_encriptacion_tc")) ?
	...
	public String bancaCuentaNumero() {
		ApiResponseMB response = TarjetaCreditoService.consultaTarjetaCreditoCache(contexto, numero());
	...
	public Boolean esAdicionalPropia() {
		TarjetaCredito tarjetaCreditoTitular = contexto.tarjetaCreditoTitular();
		return cuenta().equals(tarjetaCreditoTitular.cuenta());
	}
	...
	public String fechaVigenciaDesde(String formato) {
		try {
			if (consolidada.date("vigenciaDesde", "yyyy-MM-dd", formato) == null
					|| consolidada.date("vigenciaDesde", "yyyy-MM-dd", formato).isEmpty()) {
				ApiResponseMB response = TarjetaCreditoService.consultaTarjetaCredito(contexto, this.numero());
				return response.objetos().get(0).date("vigenciaDesde", "yyyy-MM-dd", formato);
			}
			return consolidada.date("vigenciaDesde", "yyyy-MM-dd", formato);
		} catch (Exception e) {
			return "";
		}
	}

	public String fechaVigenciaHasta(String formato) {
		try {
			if (consolidada.date("vigenciaHasta", "yyyy-MM-dd", formato) == null
					|| consolidada.date("vigenciaDesde", "yyyy-MM-dd", formato).isEmpty()) {
				ApiResponseMB response = TarjetaCreditoService.consultaTarjetaCredito(contexto, this.numero());
				return response.objetos().get(0).date("vigenciaHasta", "yyyy-MM-dd", formato);
			}
			return consolidada.date("vigenciaHasta", "yyyy-MM-dd", formato);
		} catch (Exception e) {
			return "";
		}
	}
	...
	public Boolean esOrigenVirtual() {
		return "SI".equalsIgnoreCase(consolidada.string("virtualGenerada"));
	}

	public Boolean esTarjetaVirtual() {
		return "SI".equalsIgnoreCase(consolidada.string("tarjetaVirtual"));
	}
	...
	public String numeroBin() {
		return StringUtils.isNotBlank(consolidada.string("numero")) ? consolidada.string("numero").substring(0, ConfigMB.integer("cantidad_digitos_bin_tarjeta_credito", 8)) : "";
	}
